...
class AIAnalyzer:
 ...
 def _sync_config(self):
  cfg=load(); self.provider=(self._fixed_provider or cfg.get("provider") or "gemini").lower(); self.gemini=GeminiAnalyzer(api_key=cfg.get("gemini_key") or None,model=cfg.get("gemini_model") or None); self.groq_key=cfg.get("groq_key") or os.getenv("GROQ_API_KEY"); self.groq_model=cfg.get("groq_model") or "openai/gpt-oss-20b"; self.openrouter_key=cfg.get("openrouter_key") or os.getenv("OPENROUTER_API_KEY"); self.openrouter_model=cfg.get("openrouter_model") or "openrouter/free"; configured=cfg.get("openrouter_models") or os.getenv("XFI_GUARD_OPENROUTER_MODELS",""); raw=configured if isinstance(configured,(list,tuple)) else str(configured).split(","); self.openrouter_models=[str(x).strip() for x in raw if str(x).strip()] or [self.openrouter_model]; self.weights={**DEFAULT_WEIGHTS,**{k:float(v) for k,v in (cfg.get("ai_weights") or {}).items() if k in PROVIDERS}}; self.min_consensus=float(cfg.get("ai_min_consensus",os.getenv("XFI_GUARD_MIN_CONSENSUS","0.60"))); self.request_timeout=float(cfg.get("ai_timeout",os.getenv("XFI_GUARD_AI_TIMEOUT","20"))); self.max_workers=max(1,min(8,int(cfg.get("ai_max_workers",os.getenv("XFI_GUARD_AI_MAX_WORKERS","6"))))); self.cooldown=float(cfg.get("ai_cooldown",os.getenv("XFI_GUARD_AI_COOLDOWN","30"))); return cfg
 ...
 def _jobs(self,include_cooldown=False):
  ...
  for provider in self.available_providers():
   models=[self.gemini.model] if provider=="gemini" else [self.groq_model] if provider=="groq" else self.openrouter_models; jobs += [(provider,m) for m in models]
 ...
 def _parse_verdict(text):
  ...
  try:
   raw=json.loads(text); risk=str(raw.get("risk","unknown")).lower(); conf=max(0,min(1,float(raw.get("confidence",0) or 0))); reason=str(raw.get("reason",raw.get("message",raw.get("explanation",""))))[:700]; return {"risk":risk,"confidence":conf,"reason":reason} if risk in {"low","medium","high","critical"} else None
  except Exception:return None
 def analyze_consensus(self,event):
  self._sync_config(); jobs=self._jobs()
  if not jobs:self.last_error="AI-провайдер не настроен. Добавьте ключ Gemini, Groq или OpenRouter."; return {"verdicts":[],"providers_used":0,"models_used":0,"providers":[],"models":[],"winner":"unknown","confidence":0,"consensus":False,"degraded":False,"error":self.last_error}
 ...
 def analyze(self,event,allow_fallback=True):
  ...
  for provider in order if allow_fallback else [self.provider]:
   ...
   model=self.gemini.model if provider=="gemini" else self.groq_model if provider=="groq" else self.openrouter_model; result=self._analyze_groq(event) if provider=="groq" else self._chat_model(provider,model,"Проанализируй событие VPS. Дай риск, признаки, причину и действие на русском.\n"+json.dumps(event,ensure_ascii=False))
   if result:self.last_provider=provider; self.last_model=model; return result
  if not self.available_providers():self.last_error="AI-провайдер не настроен. Добавьте ключ Gemini, Groq или OpenRouter."
  return None
 def recommend_block_ips(self,events):
  ...
  for ip,items in sorted(grouped.items(),key=lambda x:-len(x[1]))[:20]:
   ...
   if result:recommendations.append({"ip":ip,"risk":"high","confidence":min(0.99,0.60+len(items)*0.03),"reason":result[:700]})
