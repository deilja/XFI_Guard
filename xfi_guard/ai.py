...
class AIAnalyzer:
    def __init__(self, provider=None):
        ...
        self.cooldown=float(cfg.get("ai_cooldown",os.getenv("XFI_GUARD_AI_COOLDOWN","30"))); self._lock=threading.Lock(); self._failures={}
    ...
    def _chat_model(self,provider,model,prompt,json_mode=False,force=False):
        ...
        if provider=="gemini":
            try:
                result=self.gemini.analyze({"event_type":"security_analysis","message":prompt}); self.last_error=self.gemini.last_error or ""; self.last_provider=provider; self.last_model=model
    ...
    def _analyze_groq(self,event):
        return self._chat_model("groq",self.groq_model,"Проанализируй событие VPS. Дай риск, признаки, причину и действие на русском.\n"+json.dumps(event,ensure_ascii=False))
    ...
    def analyze_consensus(self,event):
        jobs=self._jobs()
        if not jobs:
            self.last_error="AI-провайдер не настроен. Добавьте ключ Gemini, Groq или OpenRouter."
