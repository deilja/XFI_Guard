...
class AIAnalyzer:
    ...
    def _chat_model(self,provider,model,prompt,json_mode=False,force=False):
        ...
        if provider=="gemini":
            try:
                result=self.gemini.analyze({"event_type":"security_analysis","message":prompt}); self.last_error=self.gemini.last_error or ""; (self._success if result else self._failure)(key_id); return result
        ...
        try:
            ...
            self._success(key_id); self.last_error=""; return result
    ...
    def analyze_consensus(self,event):
        jobs=self._jobs(); prompt="Верни ТОЛЬКО JSON-объект без markdown: {\"risk\":\"low|medium|high|critical\",\"confidence\":0..1,\"reason\":\"кратко\"}. Анализируй только факты события.\n"+json.dumps(event,ensure_ascii=False)
        def run(job):
        ...
        winner=max(scores,key=scores.get) if verdicts else "unknown"; weighted=scores[winner]/total if total else 0; agreement=sum(self.weights.get(x["provider"],1.0) for x in verdicts if x["risk"]==winner)/(total or 1); conflict=1-agreement; confidence=weighted*agreement; provider_count=len({x["provider"] for x in verdicts}); consensus=bool(verdicts) and ((provider_count>=2 and agreement>=self.min_consensus) or (provider_count==1 and confidence>=0.75))
        return {"verdicts":verdicts,"providers_used":provider_count,"models_used":len(verdicts),"providers":sorted({x["provider"] for x in verdicts}),"models":[x["model"] for x in verdicts],"winner":winner,"weighted_score":round(weighted,4),"agreement":round(agreement,4),"conflict":round(conflict,4),"confidence":round(confidence,4),"min_consensus":self.min_consensus,"consensus":consensus,"degraded":len(verdicts)<len(jobs)}
