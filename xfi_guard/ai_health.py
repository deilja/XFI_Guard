...
def run_health_check():
    """Run a real health check against every configured provider.

    Keep this function aligned with AIAnalyzer's public API. RouterAI is a
    first-class provider and therefore participates in the same check/report
    path as Gemini, Groq and OpenRouter.
    """
    analyzer = AIAnalyzer()
    models = analyzer.discover_models(force=True)
    results = []

    # Health checks must not be blocked by an earlier consensus cooldown.
