...
def _menu():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="➕ Добавить VPS"),KeyboardButton(text="🔐 Добавить по паролю")],
        [KeyboardButton(text="🗑 Удалить VPS")],[KeyboardButton(text="🔌 Подключить XFI Guard")],
        [KeyboardButton(text="🔄 Проверить VPS"),KeyboardButton(text="⬅️ Главное меню")]],resize_keyboard=True,is_persistent=True)

def _key_menu():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="✅ Доверять ключу")],[KeyboardButton(text="❌ Отмена")]],resize_keyboard=True,is_persistent=True)

def _password_menu():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="❌ Отмена")]],resize_keyboard=True,is_persistent=True)
...
def install_node_handlers(dp, admin_ids:set[int]):
    ...
    async def nodes_menu(m,state:FSMContext):
        if not ok(m): return
        await state.clear(); ns=list_configured_nodes(); rows=["🖥 VPS УЗЛЫ","",f"Настроено: {len(ns)}"]
        rows += [f"• {n.get('name')} — {n.get('user','root')}@{n.get('host')}:{n.get('port',22)}" for n in ns]
    ...
    async def add_start(m,state:FSMContext):
        if not ok(m): return
        await state.clear(); await state.update_data(password_mode=m.text=="🔐 Добавить по паролю"); await state.set_state(NodeForm.name)
        await m.answer("➕ Добавление VPS\n\nВведите имя узла, например: vps-de")
    ...
    async def remove_start(m,state:FSMContext):
        ...
        await m.answer("Введите имя VPS для удаления:\n\n"+"\n".join(f"• {x}" for x in ns))
    ...
    async def node_port(m,state:FSMContext):
        ...
        data=await state.get_data(); node=Node(name=data["name"],host=data["host"],user=data.get("user","root"),port=port)
        good,fp=await asyncio.to_thread(host_key_fingerprint,node)
        if not good: await state.clear(); return await m.answer(f"🟡 Host key не получен:\n{fp}",reply_markup=_menu())
        await state.update_data(port=port,fingerprint=fp); await state.set_state(NodeForm.hostkey_confirm)
        await m.answer(f"🔐 SSH host key\n\nVPS: {node.host}:{port}\nED25519: {fp}\n\nПроверьте fingerprint на VPS и подтвердите.",reply_markup=_key_menu())
    ...
    async def confirm_key(m,state:FSMContext):
        if not ok(m): return
        data=await state.get_data(); node=Node(name=data["name"],host=data["host"],user=data.get("user","root"),port=int(data["port"]))
        good,result=await asyncio.to_thread(enroll_host_key,node)
        if not good: await state.clear(); return await m.answer(f"❌ known_hosts: {result}",reply_markup=_menu())
        if data.get("password_mode"):
            await state.set_state(NodeForm.password); return await m.answer("🔑 Введите SSH пароль VPS.\n\nПароль используется только для первичного подключения и не сохраняется.",reply_markup=_password_menu())
        try:
            await asyncio.to_thread(add_node,node.name,node.host,node.user,node.port); await state.clear()
            await m.answer(f"✅ VPS добавлен.\n{node.user}@{node.host}:{node.port}\nSSH ключ XFI Guard не требуется вручную.",reply_markup=_menu())
        except Exception as e: await state.clear(); await m.answer(f"❌ {type(e).__name__}: {e}",reply_markup=_menu())
    ...
    async def password(m,state:FSMContext):
        ...
        await state.update_data(password=password); await state.set_state(NodeForm.password_confirm)
        await m.answer("🔐 Пароль получен. Нажмите «Подключить» для проверки и установки SSH-ключа.",reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="🔌 Подключить")],[KeyboardButton(text="❌ Отмена")]],resize_keyboard=True,is_persistent=True))

    @dp.message(F.text=="🔌 Подключить",NodeForm.password_confirm)
    async def password_connect(m,state:FSMContext):
        if not ok(m): return
        data=await state.get_data(); password=data.pop("password","")
        node=Node(name=data["name"],host=data["host"],user=data.get("user","root"),port=int(data["port"]))
        await state.update_data(**data)
        try:
            good,out=await asyncio.to_thread(bootstrap_with_password,node.host,node.user,node.port,password)
        finally: password=""
        if not good: await state.clear(); return await m.answer(f"❌ SSH подключение не выполнено:\n{out}",reply_markup=_menu())
        try: await asyncio.to_thread(add_node,node.name,node.host,node.user,node.port)
        except Exception as e: await state.clear(); return await m.answer(f"❌ SSH успешен, но узел не сохранён: {type(e).__name__}: {e}",reply_markup=_menu())
        await state.clear(); await m.answer(f"✅ VPS подключён\n\n{node.name}\nSSH: {node.user}@{node.host}:{node.port}\n\n🔑 SSH-ключ установлен.\n🔒 Пароль не сохранён.",reply_markup=_menu())
    ...
    async def bootstrap_menu(m):
        ...
        for n in ns:
            try: port=int(n.get("port",22)); good,out=await asyncio.to_thread(bootstrap,n.get("host",""),n.get("user","root"),port)
