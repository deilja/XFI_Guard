...
def _detail_menu(name): return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="🔄 Обновить")],[KeyboardButton(text="♻️ Перезапустить XFI Guard")],[KeyboardButton(text="⬅️ VPS узлы")], [KeyboardButton(text="⬅️ Главное меню")]],resize_keyboard=True,is_persistent=True)
...
def _find_node(name):
    ...
    return None
def _detail(x):
    mem=x.get("memory",""); disk=x.get("disk",""); load=x.get("load","—"); uptime=x.get("uptime","—")
    return "\n".join([f"🟢 VPS: {x.get('name','—')}","",f"Host: {x.get('host','—')}",f"Status: {str(x.get('status','offline')).upper()}",f"Hostname: {x.get('hostname','—')}",f"XFI Guard: {x.get('xfi_guard','—')}",f"Fail2Ban: {x.get('fail2ban','—')}",f"UFW: {x.get('ufw','—')}",f"Load: {load}",f"RAM: {mem or '—'}",f"Disk /: {disk or '—'}",f"Uptime: {uptime}",f"Проверено: {x.get('checked_at','—')}",f"Ошибка: {x.get('error','нет')}"])
def install_node_handlers(dp,admin_ids:set[int]):
    ...
    async def nodes_menu(m,state:FSMContext):
        ...
        for n in ns:
            await m.answer(f"🖥 {n.get('name')}",reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text=f"🔎 {n.get('name')}")]],resize_keyboard=True))
    ...
    async def node_detail(m,state:FSMContext):
        ...
        x=await asyncio.to_thread(probe_node,node); await state.clear(); await state.update_data(detail_node=name); await m.answer(_detail(x),reply_markup=_detail_menu(name))
    ...
    async def detail_refresh(m,state:FSMContext):
        ...
        if not node:return await m.answer("❌ Карточка VPS устарела.",reply_markup=_menu())
    ...
    async def restart_cancel(m,state:FSMContext):
        if ok(m): await state.set_state(None); data=await state.get_data(); node=_find_node(data.get("detail_node")); await m.answer("Отменено.",reply_markup=_detail_menu(node.name) if node else _menu())
    ...
    async def restart_confirm(m,state:FSMContext):
        ...
        if not node:return await m.answer("❌ VPS не найден.",reply_markup=_menu())
        good,result=await asyncio.to_thread(restart_guard,node)
        x=await asyncio.to_thread(probe_node,node)
        await m.answer(("🟢 " if good else "🔴 ")+result+"\n\n"+_detail(x),reply_markup=_detail_menu(node.name))
