...
class NodeForm(StatesGroup): name=State();host=State();user=State();port=State();hostkey_confirm=State();password=State();password_confirm=State();restart_confirm=State()
...
def install_node_handlers(dp,admin_ids=None):
 ...
 async def nodes_menu(m,state):
  ...
  rows += ["","SSH: ключ XFI Guard / known_hosts","Пароли после регистрации не сохраняются."];await m.answer("\n".join(rows),reply_markup=_menu())
 ...
 async def node_detail(m,state):
  if not ok(m):return
  name=m.text[2:].strip();node=_find_node(name)
  if not node:return await m.answer("❌ VPS не найден.",reply_markup=_menu())
  await state.clear();await state.update_data(detail_node=name);x=await asyncio.to_thread(probe_node,node);await m.answer(_detail(x),reply_markup=_detail_menu(name))
 @dp.message(F.text=="🔄 Обновить VPS")
 async def detail_refresh(m,state):
  if not ok(m):return
  data=await state.get_data();node=_find_node(data.get("detail_node"))
  if not node:await state.clear();return await m.answer("❌ Карточка VPS устарела.",reply_markup=_menu())
  x=await asyncio.to_thread(probe_node,node);await m.answer(_detail(x),reply_markup=_detail_menu(node.name))
 @dp.message(F.text=="♻️ Перезапустить XFI Guard")
 async def restart_start(m,state):
  if not ok(m):return
  data=await state.get_data();node=_find_node(data.get("detail_node"))
 ...
 async def restart_cancel(m,state):
  if ok(m):await state.clear();await m.answer("Отменено.",reply_markup=_menu())
 @dp.message(F.text=="✅ Да, перезапустить",NodeForm.restart_confirm)
 async def restart_confirm(m,state):
  if not ok(m):return
  data=await state.get_data();node=_find_node(data.get("detail_node"));await state.clear()
  if not node:return await m.answer("❌ VPS не найден.",reply_markup=_menu())
  good,result=await asyncio.to_thread(restart_guard,node);x=await asyncio.to_thread(probe_node,node);await m.answer(("🟢 " if good else "🔴")+(str(result)[:500])+"\n\n"+_detail(x),reply_markup=_detail_menu(node.name))
 @dp.message(F.text=="⬅️ VPS узлы")
 async def back_nodes(m,state):
  if ok(m):await state.clear();await nodes_menu(m,state)
 @dp.message(F.text.in_({"➕ Добавить VPS","🔐 Добавить по паролю"}))
 async def add_start(m,state):
  if not ok(m):return
 ...
 async def remove_start(m,state):
  if not ok(m):return
 ...
 async def node_name(m,state):
  ...
  if data.get("remove_mode"):
   ...
   except Exception:await state.clear();await m.answer("❌ Не удалось удалить VPS.",reply_markup=_menu())
   return
  await state.update_data(name=text);await state.set_state(NodeForm.host);await m.answer("Введите IP или DNS имя VPS. Можно указать с портом: 2.27.37.78:22")
 @dp.message(NodeForm.host)
 async def node_host(m,state):
  if not ok(m):return
  raw=(m.text or "").strip();host,embedded_port=_normalize_host(raw)
  if not host or not _valid_host(host):return await m.answer("❌ Некорректный IP/DNS.")
 ...
 async def node_user(m,state):
  if not ok(m):return
  text=(m.text or "").strip()
  if text and any(c.isspace() for c in text):return await m.answer("❌ Некорректный SSH пользователь.")
  await state.update_data(user=text or "root");await state.set_state(NodeForm.port);data=await state.get_data();await m.answer(f"Введите SSH порт (по умолчанию {data.get('embedded_port') or 22}):")
 @dp.message(NodeForm.port)
 async def node_port(m,state):
  if not ok(m):return
  ...
  if not good:return await m.answer("🟡 SSH host key не получен. Проверьте IP, порт и доступность SSH.")
  await state.update_data(port=port,fingerprint=fp);await state.set_state(NodeForm.hostkey_confirm);await m.answer(f"🔐 SSH host key\n\nVPS: {node.host}:{port}\nED25519: {fp}\n\nПроверьте fingerprint и подтвердите.",reply_markup=_key_menu())
 ...
 async def confirm_key(m,state):
  ...
  if not good:await state.clear();return await m.answer("❌ Не удалось добавить SSH host key.",reply_markup=_menu())
  if data.get("password_mode"):await state.set_state(NodeForm.password);return await m.answer("🔑 Введите SSH пароль VPS. Пароль используется только для первичного подключения и не сохраняется.",reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="❌ Отмена")]],resize_keyboard=True,is_persistent=True))
  try:await asyncio.to_thread(add_node,node.name,node.host,node.user,node.port)
  except Exception:await state.clear();await m.answer("❌ Не удалось сохранить VPS.",reply_markup=_menu());return
 ...
 async def password_connect(m,state):
  if not ok(m):return
  ...
  if not good:return await m.answer("❌ SSH подключение не выполнено.",reply_markup=_menu())
  try:await asyncio.to_thread(add_node,node.name,node.host,node.user,node.port,str(DEFAULT_IDENTITY_FILE))
  except Exception:return await m.answer("❌ SSH успешен, но узел не сохранён.",reply_markup=_menu())
 ...
 async def bootstrap_menu(m):
  ...
  for n in ns:
   ...
   except Exception:good,out=False,"ошибка подключения"
