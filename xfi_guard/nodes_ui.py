...
def _menu():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="➕ Добавить VPS"),KeyboardButton(text="🔐 Добавить по паролю")],[KeyboardButton(text="🗑 Удалить VPS")],[KeyboardButton(text="🔌 Подключить XFI Guard")],[KeyboardButton(text="🔄 Проверить VPS"),KeyboardButton(text="⬅️ Главное меню")]],resize_keyboard=True,is_persistent=True)

def _detail_menu(name):
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="🔄 Обновить VPS")],[KeyboardButton(text="♻️ Перезапустить XFI Guard")],[KeyboardButton(text="⬅️ VPS узлы")],[KeyboardButton(text="⬅️ Главное меню")]],resize_keyboard=True,is_persistent=True)
...
def install_node_handlers(dp,admin_ids:set[int]):
    ...
    # Global "🔄 Обновить" belongs to bot.py. Keep VPS refresh isolated to avoid handler collision.
    ...
    async def detail_refresh(m,state:FSMContext):
        ...
        if not node:
            await state.clear();return await m.answer("❌ Карточка VPS устарела. Откройте VPS заново через «🖥 VPS узлы».",reply_markup=_menu())
        x=await asyncio.to_thread(probe_node,node);await state.update_data(detail_node=node.name);await m.answer(_detail(x),reply_markup=_detail_menu(node.name))
    ...
    async def restart_confirm(m,state:FSMContext):
        ...
        if not node:return await m.answer("❌ VPS не найден.",reply_markup=_menu())
        good,result=await asyncio.to_thread(restart_guard,node);x=await asyncio.to_thread(probe_node,node);await state.update_data(detail_node=node.name);await m.answer(("🟢 " if good else "🔴 ")+result+"\n\n"+_detail(x),reply_markup=_detail_menu(node.name))
