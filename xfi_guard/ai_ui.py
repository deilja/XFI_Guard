...
class AISetupStates(StatesGroup):
    gemini_key = State(); groq_key = State(); gemini_model = State(); groq_model = State()
...
def _mask(value):
    if not value:return "не настроен"
    return value[:4]+"…"+value[-4:] if len(value)>=8 else "настроен"
def _ai_menu():
    return _kb([["🟢 Gemini","🔵 Groq"],["🔀 Выбрать AI"],["🔑 Ключ Gemini","🔑 Ключ Groq"],["🧠 Модель Gemini","🧠 Модель Groq"],["📋 Модели Gemini","📋 Модели Groq"],["✏️ Своя модель Gemini","✏️ Своя модель Groq"],["🧪 Проверить AI","📊 Диагностика AI"],["ℹ️ Статус AI"],["⬅️ Главное меню"]])
def _model_menu(provider,models):
    rows=[[f"🧠 {x}" for x in models[i:i+2]] for i in range(0,len(models),2)]; rows += [[f"🔄 Получить модели {provider} API"],[f"✏️ Своя модель {provider}"],["⬅️ AI"]]; return _kb(rows)
...
def _fetch_gemini_models(key):
    ...
    with request.urlopen(req,timeout=15) as response:data=json.loads(response.read().decode())
    return sorted({str(x.get("name","")).removeprefix("models/") for x in data.get("models",[]) if str(x.get("name","")).removeprefix("models/").startswith("gemini-") and "generateContent" in (x.get("supportedGenerationMethods",[]) or [])})

def install_ai_handlers(dp: Dispatcher) -> None:
    if getattr(dp,"_xfi_ai_handlers_installed",False):return
    dp._xfi_ai_handlers_installed=True
    @dp.message(F.text=="🟢 Gemini")
    async def select_gemini(m,state):
        if not _admin(m):return
        cfg=load();cfg["provider"]="gemini";save(cfg);await state.clear();await m.answer("🟢 Активный AI: Gemini",reply_markup=_ai_menu())
    @dp.message(F.text=="🔵 Groq")
    async def select_groq(m,state):
        if not _admin(m):return
        cfg=load();cfg["provider"]="groq";save(cfg);await state.clear();await m.answer("🔵 Активный AI: Groq",reply_markup=_ai_menu())
    ...
    async def gemini_key_prompt(m,state):
        if _admin(m):await state.set_state(AISetupStates.gemini_key);await m.answer("Введите API-ключ Gemini:",reply_markup=_kb([["❌ Отмена"],["⬅️ AI"]]))
    @dp.message(AISetupStates.gemini_key)
    async def gemini_key_save(m,state):
        if not _admin(m):return
        if m.text in {"❌ Отмена","⬅️ AI"}:await state.clear();await m.answer("Отменено.",reply_markup=_ai_menu());return
        key=(m.text or "").strip()
        if len(key)<10:await m.answer("❌ Ключ слишком короткий.");return
        cfg=load();cfg["gemini_key"]=key;cfg["provider"]="gemini";save(cfg);await state.clear();await m.answer(f"✅ Ключ Gemini сохранён: {_mask(key)}",reply_markup=_ai_menu())
    @dp.message(F.text=="🔑 Ключ Groq")
    async def groq_key_prompt(m,state):
        if _admin(m):await state.set_state(AISetupStates.groq_key);await m.answer("Введите API-ключ Groq:",reply_markup=_kb([["❌ Отмена"],["⬅️ AI"]]))
    ...
    async def groq_key_save(m,state):
        ...
        key=(m.text or "").strip()
        if len(key)<10:await m.answer("❌ Ключ слишком короткий.");return
        cfg=load();cfg["groq_key"]=key;cfg["provider"]="groq";save(cfg);await state.clear();await m.answer(f"✅ Ключ Groq сохранён: {_mask(key)}",reply_markup=_ai_menu())
    ...
    async def gemini_models(m):
        ...
        if not key:await m.answer("❌ Сначала сохраните ключ Gemini.",reply_markup=_ai_menu());return
        try:
            models=await asyncio.to_thread(_fetch_gemini_models,key);await m.answer("📋 Gemini API:\n\n"+"\n".join(models[:40]),reply_markup=_model_menu("Gemini",models[:20] or ["gemini-2.5-flash"]))
        except Exception as exc:await m.answer(f"❌ Gemini API: {type(exc).__name__}: {exc}",reply_markup=_ai_menu())
    @dp.message(F.text=="📋 Модели Groq")
    async def groq_models(m):
        if not _admin(m):return
        key=load().get("groq_key","")
        if not key:await m.answer("❌ Сначала сохраните ключ Groq.",reply_markup=_ai_menu());return
        try:
            models=await asyncio.to_thread(_fetch_groq_models,key);await m.answer("📋 Groq API:\n\n"+"\n".join(models[:40]),reply_markup=_model_menu("Groq",models[:20] or ["openai/gpt-oss-20b"]))
        except Exception as exc:await m.answer(f"❌ Groq API: {type(exc).__name__}: {exc}",reply_markup=_ai_menu())
    ...
    async def choose_gemini_model(m):
        if _admin(m):
            model=(m.text or "").removeprefix("🧠 ").strip();cfg=load();cfg["gemini_model"]=model;save(cfg);await m.answer(f"✅ Модель Gemini: {model}",reply_markup=_ai_menu())
    @dp.message(F.text.startswith("🧠 openai/"))
    async def choose_groq_openai(m):
        if _admin(m):
            model=(m.text or "").removeprefix("🧠 ").strip();cfg=load();cfg["groq_model"]=model;save(cfg);await m.answer(f"✅ Модель Groq: {model}",reply_markup=_ai_menu())
    ...
    async def custom_gemini_prompt(m,state):
        if _admin(m):await state.set_state(AISetupStates.gemini_model);await m.answer("Введите ID модели Gemini:",reply_markup=_kb([["❌ Отмена"],["⬅️ AI"]]))
    @dp.message(AISetupStates.gemini_model)
    async def custom_gemini_save(m,state):
        if not _admin(m):return
        if m.text in {"❌ Отмена","⬅️ AI"}:await state.clear();await m.answer("Отменено.",reply_markup=_ai_menu());return
        model=(m.text or "").strip();cfg=load();cfg["gemini_model"]=model;save(cfg);await state.clear();await m.answer(f"✅ Модель Gemini: {model}",reply_markup=_ai_menu())
    @dp.message(F.text=="✏️ Своя модель Groq")
    async def custom_groq_prompt(m,state):
        if _admin(m):await state.set_state(AISetupStates.groq_model);await m.answer("Введите ID модели Groq:",reply_markup=_kb([["❌ Отмена"],["⬅️ AI"]]))
    @dp.message(AISetupStates.groq_model)
    async def custom_groq_save(m,state):
        if not _admin(m):return
        if m.text in {"❌ Отмена","⬅️ AI"}:await state.clear();await m.answer("Отменено.",reply_markup=_ai_menu());return
        model=(m.text or "").strip();cfg=load();cfg["groq_model"]=model;save(cfg);await state.clear();await m.answer(f"✅ Модель Groq: {model}",reply_markup=_ai_menu())
    @dp.message(F.text=="🧪 Проверить AI")
    async def test_ai(m):
        if not _admin(m):return
        analyzer=AIAnalyzer()
        if not analyzer.enabled():await m.answer("❌ AI не настроен. Добавьте API-ключ Gemini, Groq или OpenRouter.",reply_markup=_ai_menu());return
        result=await asyncio.to_thread(run_health_check);failed=[x for x in result["results"] if not x["ok"]]
