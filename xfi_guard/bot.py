...
from .security_brain_ui import format_brain_report, format_ai_verdicts
...
def update_kb(): return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬆️ Обновить XFI Guard", callback_data="xfi_update")]])
...
def build_dispatcher():
    ...
    async def ai_test(m):
        if admin(m):
            a=AIAnalyzer(); result=await asyncio.to_thread(a.analyze_consensus,{"event_type":"health_check","severity":"info","message":"Проверка доступности AI XFI Guard"}); await m.answer("🧪 Проверка AI\n\n"+json.dumps(result,ensure_ascii=False,indent=2)[:3800],reply_markup=ai_kb())
    ...
    async def center_recommend(m,state):
        if admin(m): await state.clear(); await m.answer("🤖 Анализ кандидатов...",reply_markup=center_kb()); recs=await asyncio.to_thread(AIAnalyzer().recommend_block_ips,event_dicts()); await m.answer("\n\n".join(f"🚨 {r['ip']} — {r['risk'].upper()} ({r['confidence']:.0%})\n{r['reason']}" for r in recs)[:3900] or "Нет рекомендаций.",reply_markup=block_kb())
