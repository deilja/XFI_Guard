...
def active_events():
    """События, относящиеся только к ещё не заблокированным IP."""
...
def event_dicts():
    return [
        {"timestamp": getattr(e, "timestamp", ""), "severity": getattr(e, "severity", "unknown"),
         "event_type": getattr(e, "event_type", "unknown"), "ip": getattr(e, "ip", None),
         "user": getattr(e, "user", None), "message": getattr(e, "message", "")}
...
def attack_surface_text():
    ...
    items = [x for x in data.get("ips", []) if str(x.get("ip", "")).strip() not in {str(i).strip() for i in list_blocked_ips()}]
    lines = ["🛡 Полная картина атак за текущий период", "", f"Fail2Ban: {data.get('fail2ban_count', 0)}",
             f"UFW DENY/REJECT: {data.get('ufw_count', 0)}", f"SSH неудачных входов: {data.get('ssh_count', 0)}",
...
def fetch_gemini_models(key):
    ...
    for item in data.get("models", []):
        name = str(item.get("name", "")); model_id = name.removeprefix("models/"); methods = item.get("supportedGenerationMethods", []) or []
        if model_id.startswith("gemini-") and "generateContent" in methods and not any(x in model_id.lower() for x in ("image", "tts", "embedding", "robotics")): models.append(model_id)
...
def build_dispatcher():
    ...
    async def ev(m):
        if admin(m):
            e = active_events()[-20:]
            t = "\n".join(f"{x.severity.upper()} | {x.event_type} | {x.ip or '-'} | {x.user or '-'}" for x in e) or "Нет активных событий. Заблокированные IP скрыты из списка."
    ...
    async def choose_event_ip(m, state):
        ...
        if not counts:
            await m.answer("Нет активных публичных IPv4 в событиях. Уже заблокированные адреса не показываются.", reply_markup=block_kb()); return
    ...
    async def event_ip_button(m, state):
        if not admin(m): return
        ip = m.text.split()[1]; await state.update_data(block_ip=ip, source="events"); await state.set_state(SetupStates.confirm_block)
    ...
    async def cancel_block(m, state): await state.clear(); await m.answer("❌ Блокировка отменена.", reply_markup=block_kb())
    ...
    async def choose_unblock(m, state):
        ...
        await state.set_state(SetupStates.unblock_ip); await m.answer("Выберите IP для разблокировки:", reply_markup=kb([[f"🔓 {ip}"] for ip in ips[:20]] + [["❌ Отмена"]]))
