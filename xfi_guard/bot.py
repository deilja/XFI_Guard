...
# Актуальные текстовые Gemini-модели. ID соответствуют Gemini API.
GEMINI_MODELS = [
...
def gemini_models_kb():
    rows = [
        [x[0] for x in GEMINI_MODELS[i:i + 2]]
        for i in range(0, len(GEMINI_MODELS), 2)
    ]
    rows += [
        ["🔄 Получить модели Gemini API"],
        ["✏️ Своя модель Gemini"],
        ["⬅️ AI"],
    ]
    return kb(rows)
...
def groq_models_kb():
    rows = [
        [x[0] for x in GROQ_MODELS[i:i + 2]]
        for i in range(0, len(GROQ_MODELS), 2)
    ]
    rows += [
        ["🔄 Получить модели Groq API"],
        ["✏️ Своя модель Groq"],
        ["⬅️ AI"],
    ]
    return kb(rows)
...
def block_kb():
    return kb([
        ["🤖 Рекомендации AI"],
        ["📋 Выбрать IP из событий"],
        ["✏️ Ввести IP вручную"],
        ["📋 Заблокированные IP", "🔓 Разблокировать IP"],
        ["⬅️ Главное меню"],
    ])
...
def fetch_gemini_models(key):
    """Получить доступные текстовые модели Gemini для API-ключа."""
    req = request.Request(
    ...
    for item in data.get("models", []):
        ...
        if "generateContent" not in methods:
            continue
        low = model_id.lower()
        if any(x in low for x in ("image", "tts", "embedding", "robotics")):
            continue
...
def build_dispatcher():
    ...
    async def ev(m):
        if admin(m):
            e = events()[-20:]
            t = "\n".join(f"{x.severity.upper()} | {x.event_type} | {x.ip or '-'} | {x.user or '-'}" for x in e) or "Нет событий."
    ...
    async def choose_event_ip(m, state):
        ...
        if not counts:
            await m.answer("Нет публичных IPv4 в событиях.", reply_markup=block_kb())
            return
    ...
    async def recommended_block_button(m, state):
        ...
        await state.update_data(block_ip=ip)
        await state.set_state(SetupStates.confirm_block)
        await m.answer(f"⚠️ AI рекомендует проверить этот адрес перед блокировкой.\n\nIP: {ip}\n\nБлокировать?", reply_markup=confirm_kb())
    ...
    async def choose_unblock(m, state):
        ...
        await m.answer("Выберите IP для разблокировки:", reply_markup=kb([[f"🔓 {ip}"] for ip in ips[:20]] + [["❌ Отмена"]]))
    ...
    async def center_action(m):
        ...
        if m.text == "🤖 Рекомендации блокировки":
            recs = AIAnalyzer().recommend_block_ips(event_dicts())
            if not recs:
                await m.answer("❌ AI не дал рекомендаций по блокировке.", reply_markup=center_kb())
                return
            for r in recs:
                await m.answer(f"🚨 AI рекомендует:\n{r['ip']}\nРиск: {r.get('risk', 'medium').upper()}\nУверенность: {r['confidence']:.0%}\nПричина: {r['reason']}", reply_markup=kb([[f"🚫 Заблокировать {r['ip']}"], ["⬅️ Главное меню"]]))
            return
        s = summarize(events(), 24)
        if m.text == "📊 Анализ за 24 часа":
            t = f"📊 Анализ за 24 часа\n\nСобытий: {s['events']}\nУникальных IP: {s['unique_ips']}\nКритических: {s['critical']}\nПредупреждений: {s['warning']}"
        elif m.text == "🚨 Топ атакующих IP":
            t = "🚨 Топ атакующих IP\n\n" + ("\n".join(f"{ip}: {n} событий" for ip, n in s["top_ips"]) or "Нет данных.")
        elif m.text == "🤖 AI-анализ сводки":
            t = "🤖 AI-анализ\n\n" + (ai_report(events()) or "AI не настроен или не ответил.")
        else:
            t = f"🔄 Данные обновлены\n\nСобытий: {s['events']}\nУникальных IP: {s['unique_ips']}"
    ...
    async def provider(m, state):
        if not admin(m):
            return
        if m.text == "🔀 Выбрать AI":
            await state.set_state(SetupStates.provider)
            await m.answer("Выберите провайдера кнопкой 🟢 Gemini или 🔵 Groq.", reply_markup=ai_kb())
            return
        p = "gemini" if m.text == "🟢 Gemini" else "groq"
        c = load_ai()
        c["provider"] = p
        save_ai(c)
        await m.answer(f"✅ Активный AI: {p.upper()}", reply_markup=ai_kb())
    ...
    async def provider_text(m, state):
        ...
        if p not in {"gemini", "groq"}:
            await m.answer("Выберите 🟢 Gemini или 🔵 Groq.", reply_markup=ai_kb())
            return
        c = load_ai()
        c["provider"] = p
        save_ai(c)
    ...
    async def ask(m, state, p):
        if admin(m):
            await state.set_state(SetupStates.gemini_key if p == "gemini" else SetupStates.groq_key)
            await m.answer(f"🔑 Отправьте API-ключ {p.upper()} одним сообщением. После сохранения сообщение будет удалено.")

    @dp.message(F.text == "🔑 Ключ Gemini")
    async def gk(m, state):
        await ask(m, state, "gemini")

    @dp.message(F.text == "🔑 Ключ Groq")
    async def qk(m, state):
        await ask(m, state, "groq")
    ...
    async def savekey(m, state, p):
        ...
        k = (m.text or "").strip()
        if len(k) < 20:
            await m.answer("❌ Ключ слишком короткий или некорректный.")
            return
        c = load_ai()
        c["gemini_key" if p == "gemini" else "groq_key"] = k
        save_ai(c)
        await state.clear()
        try:
            await m.delete()
    ...
    async def sg(m, state):
        await savekey(m, state, "gemini")

    @dp.message(SetupStates.groq_key)
    async def sq(m, state):
        await savekey(m, state, "groq")

    @dp.message(F.text == "🧠 Модель Gemini")
    async def gm(m, state):
        if admin(m):
            await state.clear()
            await m.answer("🧠 Выберите модель Gemini:", reply_markup=gemini_models_kb())

    @dp.message(F.text == "📋 Модели Gemini")
    async def gemini_models(m):
        if admin(m):
            await m.answer("🧠 Доступные модели Gemini:", reply_markup=gemini_models_kb())

    @dp.message(F.text.in_({x[0] for x in GEMINI_MODELS}))
    async def gemini_model_button(m):
        if not admin(m):
            return
        model = next(v for k, v in GEMINI_MODELS if k == m.text)
        c = load_ai()
        c["gemini_model"] = model
        save_ai(c)
        await m.answer(f"✅ Модель Gemini выбрана:\n{model}", reply_markup=ai_kb())

    @dp.message(F.text == "🔄 Получить модели Gemini API")
    async def gemini_api_models(m):
        if not admin(m):
            return
        c = load_ai()
        key = c.get("gemini_key") or os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
        if not key:
            await m.answer("❌ Сначала добавьте API-ключ Gemini.", reply_markup=gemini_models_kb())
            return
        try:
            models = fetch_gemini_models(key)
        except Exception as exc:
            await m.answer(f"❌ Ошибка Gemini API: {type(exc).__name__}", reply_markup=gemini_models_kb())
            return
        if not models:
            await m.answer("❌ Gemini API не вернул подходящих текстовых моделей.", reply_markup=gemini_models_kb())
            return
        rows = [[f"🟣 {x}"] for x in models[:40]]
        rows.append(["⬅️ AI"])
        await m.answer("📋 Модели Gemini, доступные вашему API-ключу:", reply_markup=kb(rows))

    @dp.message(F.text.startswith("🟣 "))
    async def gemini_api_model_button(m):
        if not admin(m):
            return
        model = m.text[2:].strip()
        c = load_ai()
        c["gemini_model"] = normalize_model(model)
        save_ai(c)
        await m.answer(f"✅ Модель Gemini выбрана:\n{c['gemini_model']}", reply_markup=ai_kb())

    @dp.message(F.text == "✏️ Своя модель Gemini")
    async def custom_gemini_model(m, state):
        if admin(m):
            await state.set_state(SetupStates.gemini_model)
            await m.answer("Введите точный ID модели Gemini, например:\ngemini-3.1-pro-preview")

    @dp.message(F.text == "🧠 Модель Groq")
    async def gr(m, state):
        if admin(m):
            await m.answer("🧠 Выберите модель Groq:", reply_markup=groq_models_kb())

    @dp.message(F.text == "📋 Модели Groq")
    async def groq_models(m):
        if admin(m):
            await m.answer("📋 Доступные модели Groq:", reply_markup=groq_models_kb())
    ...
    async def groq_api_models(m):
        if not admin(m):
            return
        c = load_ai()
        key = c.get("groq_key") or os.getenv("GROQ_API_KEY") or os.getenv("XFI_GUARD_GROQ_API_KEY")
        if not key:
            await m.answer("❌ Сначала добавьте API-ключ Groq.", reply_markup=groq_models_kb())
            return
        try:
            models = fetch_groq_models(key)
        except Exception as exc:
            await m.answer(f"❌ Ошибка Groq API: {type(exc).__name__}", reply_markup=groq_models_kb())
            return
        if not models:
            await m.answer("❌ Groq API не вернул моделей.", reply_markup=groq_models_kb())
            return
        rows = [[f"🔹 {x}" for x in models[i:i + 2]] for i in range(0, min(len(models), 30), 2)]
        rows.append(["⬅️ AI"])
        await m.answer("📋 Доступные модели Groq API:", reply_markup=kb(rows))
    ...
    async def custom_groq_model(m, state):
        if admin(m):
            ...
            await m.answer("Введите точный ID модели Groq, например: openai/gpt-oss-120b")
    ...
    async def savemodel(m, state, p):
        ...
        if not model or any(ch.isspace() for ch in model):
            await m.answer(f"❌ Некорректный ID модели {p.upper()}.")
            return
        if p == "gemini":
            model = normalize_model(model)
        c = load_ai()
        c["groq_model" if p == "groq" else "gemini_model"] = model
        save_ai(c)
        await state.clear()
        await m.answer(f"✅ Модель {p.upper()} изменена:\n{model}", reply_markup=ai_kb())

    @dp.message(SetupStates.groq_model)
    async def sgr(m, state):
        await savemodel(m, state, "groq")

    @dp.message(SetupStates.gemini_model)
    async def sgm(m, state):
        await savemodel(m, state, "gemini")

    @dp.message(F.text == "🧪 Проверить AI")
    async def test(m):
        if admin(m):
            a = AIAnalyzer()
            r = a.analyze({"event_type": "manual_test", "severity": "warning", "message": "Тест подключения XFI Guard AI"})
...
async def main():
    if not TOKEN or not ADMIN_IDS:
        raise RuntimeError("Не заданы XFI_GUARD_BOT_TOKEN и XFI_GUARD_ADMIN_IDS")
