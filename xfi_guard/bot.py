...
def blocked_view():
    try:
        ...
        lines=["🛡 АКТИВНЫЕ БЛОКИРОВКИ","",f"Всего: {len(ips)}","Срок автоматической блокировки: 7 дней","Backend: Fail2Ban + UFW","", "IP:"]
        if ips:
            lines.extend(f"• {ip}" for ip in ips[:100])
            if len(ips)>100: lines.append(f"… ещё {len(ips)-100} IP")
        else:
            lines.append("• нет активных публичных IP-блокировок")
        return "\n".join(lines)[:3900]
    except Exception as exc:
        return f"❌ Не удалось получить список блокировок: {type(exc).__name__}: {exc}"
...
def build_dispatcher():
    ...
    def event_view():
        try:
            p=subprocess.run(["journalctl","-u","xfi-guard","-u","xfi-guard-bot","-n","25","--no-pager"],text=True,capture_output=True,timeout=8,check=False); text=(p.stdout or p.stderr).strip()[-2600:] or "Событий нет."
    ...
    async def attack_surface_button(message):
        if not admin(message): return
        data=collect_attack_surface(); text=["🛡 Полная картина атак","",f"Fail2Ban: {data.get('fail2ban_count',0)}",f"UFW DENY/REJECT: {data.get('ufw_count',0)}",f"SSH неудачных входов: {data.get('ssh_count',0)}",f"Уникальных IP: {len(data.get('ips',[]))}",""]
        for x in data.get("ips",[])[:15]: text.append(f"• {x.get('ip','-')} — {x.get('risk','unknown')} ({x.get('risk_score',0)}/100), событий: {x.get('events',0)}")
