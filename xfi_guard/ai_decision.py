...
def create(event: dict, consensus: dict) -> dict:
    ...
    normalized={"ip":event.get("ip"),"risk_score":event.get("risk_score",event.get("score")),"risk":event.get("risk"),"events":event.get("events"),"sources":event.get("sources"),"reasons":event.get("reasons"),"providers":providers,"consensus":bool(consensus.get("consensus")),"providers_used":consensus.get("providers_used",len(providers))}
...
def get(decision_id: str):
    return next((x for x in reversed(_load().get("decisions",[])) if x.get("id")==decision_id),None)
