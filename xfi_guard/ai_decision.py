...
def _load(state_file=STATE_FILE):
    try:
...
def _save(data,state_file=STATE_FILE):
...
def create(event:dict,consensus:dict,state_file=None)->dict:
    ...
    data=_load(target); data.setdefault("decisions",[]).append(record); data["decisions"]=data["decisions"][-500:]; _save(data,target); return record
