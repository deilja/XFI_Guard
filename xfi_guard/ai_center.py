...
def ai_center_menu():
    return _kb([
        ["🩺 Здоровье AI", "🧪 Проверить все AI"],
        ["🔑 API ключи", "🧩 API модели"],
        ["📊 Консенсус AI", "🔄 Синхронизация AI"],
        ["🧹 Сброс здоровья AI"],
        ["⬅️ Главное меню"],
    ])
...
def build_health_report(data: dict) -> str:
    ...
    for item in data.get("results") or []:
        ...
        lines.append(
            f"{mark} {item.get('provider')}/{item.get('model')}: "
            f"{item.get('latency_ms', 0)} ms{err}"
        )
    total = len(data.get("results") or [])
    working = sum(bool(x.get("ok")) for x in data.get("results") or [])
    lines += ["", f"Рабочих AI: {working}/{total}"]
    return "\n".join(lines)[:3900]
...
def consensus_report(status: dict) -> str:
    ...
    lines = [
        ...
        f"Настроены: {', '.join(status.get('configured_providers') or []) or 'нет'}",
        f"Участвуют сейчас: {providers}",
    ...
    if health:
        ...
        for key, value in list(health.items())[-8:]:
            lines.append(
                f"• {key}: {value.get('success_rate', 0):.0%}, "
                f"ошибок {value.get('errors', 0)}"
            )
    return "\n".join(lines)[:3900]
...
def install_ai_center_handlers(dp: Dispatcher) -> None:
    ...
    async def health(m):
        ...
        try:
            await m.answer(
                build_health_report(await asyncio.to_thread(run_health_check)),
                reply_markup=ai_center_menu(),
            )
        except Exception as exc:
            await m.answer(
                f"❌ AI health: {type(exc).__name__}: {exc}",
                reply_markup=ai_center_menu(),
            )
    ...
    async def check_all(m):
        ...
        try:
            ...
            await m.answer(
                f"❌ Проверка AI: {type(exc).__name__}: {exc}",
                reply_markup=ai_center_menu(),
            )
    ...
    async def sync(m):
        ...
        try:
            ...
            cfg.update({
                ...
                "openrouter_model": status["openrouter_model"],
                "openrouter_models": tuple(status["openrouter_models"]),
                "routerai_model": status.get("routerai_model", ""),
                "routerai_models": tuple(status.get("routerai_models") or []),
            })
            save(cfg)
            await m.answer(
                "🔄 AI синхронизирован\n\n"
                ...
                + f"Платный fallback: {'включён' if status.get('routerai_allow_paid') else 'выключен'}",
                reply_markup=ai_center_menu(),
            )
        except Exception as exc:
            await m.answer(
                f"❌ Синхронизация AI: {type(exc).__name__}: {exc}",
                reply_markup=ai_center_menu(),
            )
    ...
    async def consensus(m):
        if _admin(m):
            await m.answer(
                consensus_report(AIAnalyzer().status()),
                reply_markup=ai_center_menu(),
            )
    ...
    async def reset(m):
        if not _admin(m):
            return
        AIAnalyzer().reset_health()
        await m.answer(
            "🧹 Локальные cooldown/failure-состояния AI сброшены.",
            reply_markup=ai_center_menu(),
        )
