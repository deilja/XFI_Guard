...
import hashlib, ipaddress, json, os, time, uuid
from .ai import AIAnalyzer
from .auto_defense import ai_block
from .firewall import list_blocked_ips
from .security_db import SecurityDB
class AutoBlocker:
    def __init__(self, *, enabled=False, confidence=0.90, min_attempts=5, min_providers=2, db_path="/var/lib/xfi-guard/security.db", attempt_window_seconds=600):
        self.enabled=enabled; self.confidence=max(0.0,min(1.0,float(confidence))); self.min_attempts=max(1,int(min_attempts)); self.min_providers=max(2,int(min_providers)); self.attempt_window_seconds=max(60,int(attempt_window_seconds)); self.ai=AIAnalyzer(); self.db=SecurityDB(db_path); self.whitelist=self._load_whitelist()
    @staticmethod
    def _load_whitelist()->set[str]:
        raw=os.getenv("XFI_GUARD_AUTO_BLOCK_WHITELIST",""); result=set()
        for value in raw.split(","):
            value=value.strip()
            if not value: continue
            try:
                ip=ipaddress.ip_address(value)
                if ip.version==4: result.add(ip.compressed)
            except ValueError: continue
        return result
    @staticmethod
    def _decision_id(ip:str,analysis:dict,attempts:int)->str:
        payload={"ip":ip,"attempts":attempts,"winner":analysis.get("winner"),"confidence":analysis.get("confidence"),"providers":sorted(str(p) for p in (analysis.get("providers") or [])),"verdicts":analysis.get("verdicts") or []}
        digest=hashlib.sha256(json.dumps(payload,sort_keys=True,ensure_ascii=False,default=str,separators=(",", ":")).encode()).hexdigest()[:24]
        return f"ai-{int(time.time())}-{digest}-{uuid.uuid4().hex[:8]}"
    def evaluate(self,events):
        grouped={}
        for event in events:
            if event.get("event_type")!="ssh_auth_failed": continue
            ip=str(event.get("ip") or "").strip()
            try: parsed=ipaddress.ip_address(ip)
            except ValueError: continue
            if parsed.version!=4 or not parsed.is_global or parsed.compressed in self.whitelist: continue
