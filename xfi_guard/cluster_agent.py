...
def run(config: dict) -> None:
    ...
    master = os.getenv("XFI_GUARD_CLUSTER_MASTER_URL", cluster.get("master_url", ""))
    secret = os.getenv("XFI_GUARD_CLUSTER_SECRET", cluster.get("secret", ""))
    node = os.getenv("XFI_GUARD_CLUSTER_NODE_ID", cluster.get("node_id", os.uname().nodename))
    token = os.getenv("XFI_GUARD_CLUSTER_TOKEN", cluster.get("token", ""))
    enabled = cluster.get("enabled", True) if cluster else bool(master and secret)
    if not enabled:
    ...
    while True:
        try:
            ...
            LOG.info("cluster heartbeat: node=%s blocks_applied=%s", node, applied)
