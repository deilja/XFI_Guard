...
def create_backup(paths: list[str], label: str = "remediation") -> Path:
    """Create a private timestamped backup with a manifest."""
...
def snapshot_network() -> Path:
    """Capture current network state for an emergency rollback."""
    ...
    with (NETWORK_BACKUP / "iptables.bak").open("w", encoding="utf-8") as handle:
        subprocess.run(["iptables-save"], stdout=handle, stderr=subprocess.DEVNULL, check=False)
    return NETWORK_BACKUP


def schedule_network_rollback(minutes: int = 15) -> str:
    """Schedule a bounded network rollback using `at`."""
    if minutes < 1:
