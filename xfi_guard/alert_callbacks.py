...
def _mask(key): return key[:4]+"…"+key[-4:] if len(key)>=8 else "настроен"
async def _safe_callback_answer(callback:CallbackQuery,text:str|None=None,*,show_alert:bool=False)->bool:
    try:
        ...
        raise
def _ai_keyboard():
    return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="🟢 Gemini"),KeyboardButton(text="🔵 Groq"),KeyboardButton(text="🟣 OpenRouter")],[KeyboardButton(text="🔀 Все AI вместе")],[KeyboardButton(text="🔑 Ключ Gemini"),KeyboardButton(text="🔑 Ключ Groq")],[KeyboardButton(text="🔑 Ключ OpenRouter")],[KeyboardButton(text="🧠 Модель Gemini"),KeyboardButton(text="🧠 Модель Groq")],[KeyboardButton(text="🧠 Модель OpenRouter")],[KeyboardButton(text="🧪 Проверить AI"),KeyboardButton(text="ℹ️ Статус AI")],[KeyboardButton(text="⬆️ Обновить XFI Guard")],[KeyboardButton(text="⬅️ Главное меню")]],resize_keyboard=True,is_persistent=True)
def _back_ai_keyboard(): return ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="⬅️ AI")]],resize_keyboard=True,is_persistent=True)
def _fetch_openrouter_models(key):
    ...
    return sorted({str(x.get("id","")).strip() for x in data.get("data",[]) if "/" in str(x.get("id",""))})
...
def register_alert_callbacks(dp,admin_ids):
    ...
    async def block_alert(callback):
        ...
        if not _authorized(callback):await _safe_callback_answer(callback,"Нет доступа",show_alert=True);return
    ...
    async def block_all_critical_alert(callback):
        uid=callback.from_user.id if callback.from_user else 0
        if not _authorized(callback):await _safe_callback_answer(callback,"Нет доступа",show_alert=True);return
        items=_critical_alerts()
        if not items:await _safe_callback_answer(callback,"Критических угроз для блокировки нет",show_alert=True);return
    ...
    async def confirm_all_critical(callback):
        uid=callback.from_user.id if callback.from_user else 0
        if not _authorized(callback):await _safe_callback_answer(callback,"Нет доступа",show_alert=True);return
        ...
        _bulk_running.add(uid);items=pending[0];ok_count=0;failed=[]
        try:
            for alert in items:
                ip=str(alert.get("ip","")).strip();metadata={"decision_id":alert.get("decision_id"),"alert_id":alert.get("id"),"risk_score":alert.get("score"),"bulk":True}
                try:
                    ...
                    if ok:ok_count+=1
                    else:failed.append(f"{ip}: {str(message)[:160]}")
                except (ValueError,OSError):failed.append(f"{ip}: операция не выполнена")
    ...
    async def cancel_all_critical(callback):
        if not _authorized(callback):return await _safe_callback_answer(callback,"Нет доступа",show_alert=True)
    ...
    async def confirm_alert(callback):
        uid=callback.from_user.id if callback.from_user else 0
        if not _authorized(callback):await _safe_callback_answer(callback,"Нет доступа",show_alert=True);return
        pending=_pending.pop(uid,None)
        if not pending or pending[1]<time.monotonic():await _safe_callback_answer(callback,"Подтверждение истекло",show_alert=True);return
        ip,_,decision_id=pending;alert=next((x for x in reversed(_load().get("alerts",[])) if x.get("ip")==ip),{});decision_id=decision_id or alert.get("decision_id")
        ...
        except (ValueError,OSError):ok,message=False,"Операция не выполнена."
    ...
    async def cancel_alert(callback):
        if not _authorized(callback):return await _safe_callback_answer(callback,"Нет доступа",show_alert=True)
        _pending.pop(callback.from_user.id if callback.from_user else 0,None);await _safe_callback_answer(callback,"Отменено");await callback.message.edit_reply_markup(reply_markup=None)
    @dp.callback_query(F.data.startswith("xfi:ignore:"))
    async def ignore_alert(callback):
        if not _authorized(callback):await _safe_callback_answer(callback,"Нет доступа",show_alert=True);return
    ...
    async def detail_alert(callback):
        if not _authorized(callback):await _safe_callback_answer(callback,"Нет доступа",show_alert=True);return
        alert_id=callback.data.split(":",2)[2];alert=next((x for x in reversed(_load().get("alerts",[])) if x.get("id")==alert_id),None)
        if not alert:await _safe_callback_answer(callback,"Тревога не найдена",show_alert=True);return
    ...
    async def manual_update(message,state:FSMContext):
        ...
        try:
            ...
            if result.returncode!=0:await message.answer("❌ Не удалось запустить обновление XFI Guard.",reply_markup=_ai_keyboard());return
        ...
        except (OSError,subprocess.SubprocessError):await message.answer("❌ Не удалось запустить обновление XFI Guard.",reply_markup=_ai_keyboard())
    ...
    async def openrouter_key_save(message,state:FSMContext):
        ...
        try:
            ...
            cfg=load_ai();cfg["openrouter_key"]=key;cfg["provider"]="openrouter";cfg["openrouter_models"]=models;current=cfg.get("openrouter_model") or ""
            if current not in models:
                preferred=[x for x in models if x in {"openai/gpt-oss-20b","openai/gpt-oss-120b","google/gemini-2.5-flash","deepseek/deepseek-chat-v3-0324"}];cfg["openrouter_model"]=preferred[0] if preferred else models[0]
        ...
        except Exception:await state.clear();await message.answer("❌ OpenRouter не принял ключ или API недоступен.",reply_markup=_ai_keyboard())
    ...
    async def openrouter_model_menu(message,state:FSMContext):
        ...
        if not models and (cfg.get("openrouter_key") or os.getenv("OPENROUTER_API_KEY")):
            ...
            except Exception:await message.answer("❌ Не удалось получить список моделей.",reply_markup=_ai_keyboard());return
