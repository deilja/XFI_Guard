...
def register_alert_callbacks(dp,admin_ids):
    ...
    async def openrouter_provider(message, state:FSMContext):
        if not message.from_user or message.from_user.id not in admin_ids: return
        await state.clear()
        cfg=load_ai(); key=cfg.get("openrouter_key") or os.getenv("OPENROUTER_API_KEY","")
        if not key:
            await message.answer("🟣 OpenRouter не настроен.\n\nСначала нажмите «🔑 Ключ OpenRouter». ", reply_markup=_ai_keyboard()); return
        cfg["provider"]="openrouter"; save_ai(cfg)
        await message.answer(f"🟣 OpenRouter выбран как основной AI.\n\nКлюч: {_mask(key)}\nМодель: {cfg.get('openrouter_model','openai/gpt-oss-20b')}", reply_markup=_ai_keyboard())
    ...
    async def openrouter_key_save(message, state:FSMContext):
        ...
        if not key or len(key)<20:
            await message.answer("❌ Ключ выглядит некорректно. Введите OpenRouter API-ключ ещё раз.", reply_markup=_back_ai_keyboard()); return
        try:
    ...
    async def openrouter_model_menu(message, state:FSMContext):
        ...
        if not models:
            await message.answer("🧠 Модель OpenRouter\n\nСначала добавьте ключ OpenRouter.", reply_markup=_ai_keyboard()); return
        models=models[:60]; rows=[[KeyboardButton(text="⬅️ AI")]]+[[KeyboardButton(text=("✅ " if x==cfg.get("openrouter_model") else "")+x)] for x in models]
        await state.set_state(OpenRouterStates.model); await message.answer("🧠 Выберите модель OpenRouter:", reply_markup=ReplyKeyboardMarkup(keyboard=rows,resize_keyboard=True,is_persistent=True))
    ...
    async def openrouter_model_save(message, state:FSMContext):
        ...
        if value not in models:
            await message.answer("❌ Такой модели нет в списке."); return
        cfg["openrouter_model"]=value; cfg["provider"]="openrouter"; save_ai(cfg); await state.clear()
        await message.answer(f"✅ Модель OpenRouter выбрана:\n{value}", reply_markup=_ai_keyboard())
