...
def _consensus(risk="high", confidence=0.95, consensus=True, providers_used=2):
    ...
    return {"verdicts": verdicts, "providers_used": providers_used, "models_used": len(verdicts), "providers": providers, "models": ["test"] * len(verdicts), "winner": risk, "confidence": confidence, "consensus": consensus}
...
def test_high_multi_provider_consensus_blocks_and_audits(tmp_path, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(auto_blocker, "confirm_block", lambda ip, actor, reason, metadata: (calls.append((ip, actor, reason, metadata)) or (True, f"IP {ip} blocked in test")))
    ...
    assert rows == [("ssh_ai_check", "8.8.8.8", "high", 0.95, 5), ("auto_block", "8.8.8.8", "high", 0.95, 5)]
...
def test_single_provider_never_blocks(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(auto_blocker, "list_blocked_ips", lambda: [])
    monkeypatch.setattr(auto_blocker, "confirm_block", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("block must not be called")))
...
def test_low_risk_never_blocks_even_with_confidence(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(auto_blocker, "list_blocked_ips", lambda: [])
    monkeypatch.setattr(auto_blocker, "confirm_block", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("low risk must not be blocked")))
